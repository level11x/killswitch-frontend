...
function App() {
  ...
  try {
    window.ethereum
      .enable()
      .then((result) => console.log("enable result", result));
  } catch (err) {
    console.log(err);
  }
  ...
  const fetch = useCallback(async () => {
    ...
    const result = await lpContract.methods
      .allowance(acc, tarContractAddress)
      .call();
  ...
  const approve = () => {
    ...
    if (lpContract) {
      lpContract.methods
        .approve(tarContractAddress, amount)
        .send(options)
        .on("error", (error) => {
          if (error && error.message) {
            setTransactionState(error.message);
          } else {
            setTransactionState("Something went wrong");
          }
        })
        .on("transactionHash", (transactionHash) =>
          setTransactionState("Submitted transaction: " + transactionHash)
        )
        .on("receipt", async (receipt) => {
          setTransactionState(
            "Successfully approve contract, you can now stake LP"
          );
          const result = await lpContract.methods
            .allowance(acc, tarContractAddress)
            .call();
  ...
  async function calBalanceAndReward() {
    ...
    let pendingReward =
      (userInfo.amount * poolInfo.accCakePerShare) / userInfo.rewardDebt;
    ...
    <div className="App">
      <header className="App-header">
        ...
        <p style={{ textDecoration: "underline" }}>
          !!! Disclaimer (Alpha V0.0.1) -
          ลองกดเล่นได้ครับแต่ความเสี่ยงสูงที่จะมี Bug นะครับ !!!
          <br />{" "}
        </p>
        <li>ตอนนี้ใช้ได้เฉพาะ lp CAKE-BNB เท่านั้นนะครับ </li>
        <li>
          ต้องทำ lp CAKE-BNB ก่อนที่จะใช้งานนะครับไม่งั้นจะ ERROR Web น่าจะพัง
          เดี๋ยวจะแก้ให้ใน Version หน้าครับ
        </li>
        <li>
          ถ้าอยากลองลองแค่นิดเดียวก่อนนะครับ จริงๆ ทีมงานสามารถคืน LP
          ให้ทุกคนได้หากมีอะไรผิดพลาด แต่ทางที่ดีอย่าเพิ่งรีบลองเยอะๆ นะครับ{" "}
        </li>
        <li>
          Bonus : การ liquidate position ประหยัดค่า Gas
          กว่าการทำปกติครับในอนาคตถ้ามีการ Optimize จะประหยัดค่า Gas มากกว่าปกติ
          อย่างน้อยครึ่งนึงครับ{" "}
        </li>
        <br />
        {allowance === 0 ? (
          <button onClick={approve} style={{ color: "#008CBA" }}>
            Approve
          </button>
        ) : (
          <div>
            <button onClick={stake} style={{ backgroundColor: "#008CBA" }}>
              Stake {(walletLp / 10 ** 18).toFixed(3)} LP
            </button>
        ...
        <p>
          🏄 You have staked {(stakedLp / 10 ** 18).toFixed(3)} LP +{" "}
          {(reward / 10 ** 18).toFixed(3)} Reward 🏂
        </p>
        {allowance !== 0 ? (
          <div>
            <p>
              ⛔⛔⛔ 1) กด Killswitch ต่อเมื่อมั่นใจว่ามีค่าใน You have staked
              ด้านบนแล้วเท่านั้นนะครับไม่งั้นจะเสีย Gas ฟรีๆ ⛔⛔⛔
            </p>
            <p>
              ⛔⛔⛔ 2) กด Killswitch แล้ว LP จะหายไปทันที รวมถึง REWARD ที่ได้
              (CAKE) และจะกลายเป็น BNB เลยครับอัตโนมัติ ⛔⛔⛔
            </p>
            <p>
              ⛔⛔⛔ 3) ทิ้งไว้สัก 1-2 วันแล้วค่อยมากดก็ได้ครับ Killswitch
              ไม่ต้องรีบกด จะได้เห็น BNB ว่าเพิ่มขึ้นครับ :) ⛔⛔⛔
            </p>
            <button onClick={liquidate} style={{ backgroundColor: "#008CBA" }}>
              KillSwitch (Liquidate all position)
            </button>
          </div>
        ) : null}
        <br />
